...
function mostrar(secaoId) { document.querySelectorAll('main .card').forEach((el) => { if (el.id) el.classList.add('hidden'); }); if (secaoId) document.getElementById(secaoId)?.classList.remove('hidden'); }
...
function initSergio() {
  ...
  chat.addEventListener('click', (event) => { const target = event.target.closest('[data-question]'); if (!target) return; fecharPaineisOpcoes(); enviarPerguntaDireta(target.dataset.question || ''); });
...
async function init() { await carregarDados(); await configurarAvatarSergio(); initNavegacao(); initFaq(); initOpcoesRapidas(); initSergio(); }
